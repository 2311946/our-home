...
async function sendGroupMsg(text){let contextCount=parseInt(localStorage.getItem('ctx_count'))||30;
  if(!text)return;
...
let allChars=['yan','peiji','shenyan','axun','jiangsu','su','zouzheng','keke'];let responders=[];if(text.includes('@全体')){responders=shuffle(allChars);}else{let mentioned=allChars.filter(id=>text.includes('@'+(charNames[id]||'')));if(mentioned.length>0){responders=mentioned;}else{let count=Math.floor(Math.random()*3)+1;responders=shuffle(allChars).slice(0,count);}}for(let c of responders){if(!prompts[c])continue;chats.group.push({role:'ai',content:'',character:c,time:nowTime()});render();let ctxCount=parseInt(localStorage.getItem('ctx_count'))||30;let msgs=[{role:'system',content:prompts[c]+'\n\n当前真实时间：'+aiNowTime()+'。\n\n【这是群聊，参与者有：宣宣、言言、裴寂、沈晏。你回应任何人的话。不要说自己是AI，直接以角色身份回复。如果你想发多条消息，用 ||| 分隔每条消息。例如：第一句话|||第二句话|||第三句话。不要在回复里加时间戳或自己的名字前缀。】'}];chats.group.slice(-ctxCount).forEach(m=>{if(m.role==='user')msgs.push({role:'user',content:'宣宣: '+m.content});else if(m.role==='ai'&&m.content){if(m.character===c)msgs.push({role:'assistant',content:m.content});else msgs.push({role:'user',content:(charNames[m.character]||'')+': '+m.content});}});let idx=chats.group.length-1;try{let api = getApiForChar(c);
