  ...
  try{
    ...
    data.forEach(m=>{
      ...
      if(m.character==='group'){
        cloud.group.push({role:'user',content:m.content,time:fmtChatTime(m.msg_time)});
        ...
        cloud[m.character].push({role:m.role,content:m.content,time:fmtChatTime(m.msg_time)});
      }
    });
    // 取多的那个版本
